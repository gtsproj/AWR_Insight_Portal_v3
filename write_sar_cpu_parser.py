...
content = r'''# modules/sar/sar_cpu_parser.py  (v4 — definitive)
...
dest = os.path.join("modules", "sar", "sar_cpu_parser.py")
with open(dest, "w", encoding="utf-8") as f:
    f.write(content)

# Verify
written = open(dest, encoding="utf-8").read()
ok1 = "CPU parser -- first 20 lines" in written
ok2 = "_strip_time_prefix" in written
ok3 = "v4" in written

print(f"File written: {os.path.abspath(dest)}")
print(f"Size: {len(written)} bytes")
print(f"Has diagnostic logging: {ok1}")
print(f"Has _strip_time_prefix: {ok2}")
print(f"Has v4 marker: {ok3}")
print()
if ok1 and ok2:
    print("SUCCESS - v4 is now on disk")
else:
    print("ERROR - file content unexpected")
