#modules/io_stat_by_function_summary_parser.py
...
log_file = os.path.join(LOG_DIR, "io_stat_by_function_summary_parser.log")

logger = get_logger("io_stat_by_function_summary_parser")  # or dynamic module name


def parse_io_stat_by_function_summary(filepath):
    """Parse IOStat by Function summary section from an AWR HTML report."""
    records = []
    ...
    section = soup.find("h3", string="IOStat by Function summary")
    if not section:
        logger.warning("⚠️ IOStat by Function summary section not found.")
        return []

    table = section.find_next("table")
    if not table:
        logger.warning("⚠️ IOStat by Function summary table not found.")
        return []

    df = pd.read_html(str(table))[0]

    if is_section_empty(df, "IOStat by Function summary", "awr_iostat_function"):
    ...
    logger.info(f"✅ Parsed {len(records)} IOStat by Function summary records.")
    return records


def insert_io_stat_by_function_summary(records):
    """Insert IOStat by Function summary records into PostgreSQL."""
    if not records:
        logger.warning("⚠️ No records to insert into awr_iostat_function")
        return
    ...
    try:
        ...
        insert_query = """
            INSERT INTO awr_iostat_function (
                dbname, instance, instnum, snap_time, function_name,
        ...
        for rec in records:
            cur.execute(insert_query, (
                ...
                rec["function_name"], rec["reads_mb"], rec["reads_reqs_per_sec"],
        ...
        cur.close()
        conn.close()
        logger.info(f"✅ Inserted {len(records)} rows into awr_iostat_function")

    except psycopg2.errors.UniqueViolation:
        logger.warning("⚠️ Skipped duplicate IOStat by Function summary record")
        if conn:
            conn.rollback()

    except Exception as e:
        logger.error(f"❌ Failed inserting IOStat by Function summary: {e}", exc_info=True)
    finally:
        if conn:
            conn.close()
...
if __name__ == "__main__":
    ...
    if _target:
        records = parse_io_stat_by_function_summary(_target)
        insert_io_stat_by_function_summary(records)
    else:
