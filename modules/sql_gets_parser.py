...
log_file = os.path.join(LOG_DIR, "sql_gets_parser.log")

logger = get_logger("sql_gets_parser")  # or dynamic module name

def parse_sql_gets(filepath):
    """Parse the SQL ordered by Gets section from AWR report."""
    records = []
    ...
    section = soup.find("h3", string="SQL ordered by Gets")
    if not section:
        logger.warning("⚠️ SQL ordered by Gets section not found.")
        return []

    table = section.find_next("table")
    if not table:
        logger.warning("⚠️ SQL ordered by Gets table not found.")
        return []

    df = pd.read_html(str(table))[0]
    logger.info(f"Found {len(df)} rows in SQL Gets section")

    if is_section_empty(df, "SQL ordered by Gets", "awr_sql_gets"):
    ...
    logger.info(f"✅ Parsed {len(records)} SQL Gets records")
    return records


def insert_sql_gets(records):
    """Insert parsed SQL Gets stats into DB."""
    if not records:
        logger.warning("⚠️ No records to insert into awr_sql_gets")
        return
    ...
    try:
        ...
        conn.commit()
        cur.close()
        logger.info(f"✅ Inserted {len(records)} rows into awr_sql_gets")

    except psycopg2.errors.UniqueViolation:
        logger.warning("⚠️ Skipped duplicate SQL ordered by Gets record")
        if conn:
            conn.rollback()

    except Exception as e:
        logger.error(f"❌ Failed inserting SQL ordered by Gets: {e}", exc_info=True)
    finally:
        if conn:
            conn.close()
...
if __name__ == "__main__":
    ...
    if _target:
        records = parse_sql_gets(_target)
        insert_sql_gets(records)
    else:
