#modules/awr_seg_phy_write_req_parser.py
...
log_file = os.path.join(LOG_DIR, "awr_seg_phy_write_req_parser.log")

logger = get_logger("awr_seg_phy_write_req_parser")  # or dynamic module name


def parse_awr_seg_phy_write_req(filepath):
    """Parse the Segments by Physical Write Requests section from AWR report."""
    records = []
    ...
    section = soup.find("h3", string="Segments by Physical Write Requests")
    if not section:
        logger.warning("⚠️ Segments by Physical Write Requests section not found.")
        return []

    table = section.find_next("table")
    if not table:
        logger.warning("⚠️ Segments by Physical Write Requests table not found.")
        return []

    df = pd.read_html(str(table))[0]
    logger.info(f"Found {len(df)} rows in Segments by Physical Write Requests section")

    if is_section_empty(df, "Segments by Physical Write Requests", "awr_seg_phy_write_req"):
    ...
    for _, row in df.iterrows():
        rec = {
            ...
            "phys_write_requests": clean_number(row.get("Phys Write Requests", None)),
            "pcttotal": clean_number(row.get("%Total", None)),
        }
        rec["row_hash"] = row_hash(rec)
        records.append(rec)

    logger.info(f"✅ Parsed {len(records)} Segments by Physical Write Requests records")
    return records


def insert_awr_seg_phy_write_req(records):
    """Insert parsed Segments by Physical Write Requests into DB."""
    if not records:
        logger.warning("⚠️ No records to insert into awr_seg_phy_write_req")
        return
    ...
    try:
        ...
        conn.commit()
        cur.close()
        logger.info(f"✅ Inserted {len(records)} rows into awr_seg_phy_write_req")

    except psycopg2.errors.UniqueViolation:
        logger.warning("⚠️ Skipped duplicate Segments by Physical Write Requests record")
        if conn:
            conn.rollback()

    except Exception as e:
        logger.error(f"❌ Failed inserting Segments by Physical Write Requests: {e}", exc_info=True)
    finally:
        if conn:
            conn.close()
