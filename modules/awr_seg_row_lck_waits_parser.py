#modules/awr_seg_row_lck_waits_parser.py
...
log_file = os.path.join(LOG_DIR, "awr_seg_row_lck_waits_parser.log")


logger = get_logger("awr_seg_row_lck_waits_parser")  # or dynamic module name


def parse_awr_seg_row_lck_waits(filepath):
    """Parse the Segments by Row Lock Waits section from AWR report."""
    records = []
    ...
    section = soup.find("h3", string="Segments by Row Lock Waits")
    if not section:
        logger.warning("⚠️ Segments by Row Lock Waits section not found.")
        return []
    ...
    for _, row in df.iterrows():
        rec = {
            ...
            "row_lock_waits": clean_number(row.get("Row Lock Waits", None)),
            "pct_of_capture": clean_number(row.get("% of Capture", None)),
        }
        rec["row_hash"] = row_hash(rec)
        records.append(rec)

    logger.info(f"✅ Parsed {len(records)} Segments by Row Lock Waits records")
    return records


def insert_awr_seg_row_lck_waits(records):
    """Insert parsed Segments by Row Lock Waits into DB."""
    if not records:
        logger.warning("⚠️ No records to insert into awr_seg_row_lck_waits")
        return
    ...
    try:
        ...
        conn.commit()
        cur.close()
        logger.info(f"✅ Inserted {len(records)} rows into awr_seg_row_lck_waits")

    except psycopg2.errors.UniqueViolation:
        logger.warning("⚠️ Skipped duplicate Segments by Row Lock Waits record")
        if conn:
            conn.rollback()

    except Exception as e:
        logger.error(f"❌ Failed inserting Segments by Row Lock Waits: {e}", exc_info=True)
    finally:
        if conn:
            conn.close()
...
if __name__ == "__main__":
    ...
    if _target:
        records = parse_awr_seg_row_lck_waits(_target)
        insert_awr_seg_row_lck_waits(records)
    else:
