#modules/awr_seg_phy_writes_parser.py
...
log_file = os.path.join(LOG_DIR, "awr_seg_phy_writes_parser.log")

logger = get_logger("awr_seg_phy_writes_parser")  # or dynamic module name

def parse_awr_seg_phy_writes(filepath):
    """Parse the Segments by Segments by Physical Writes section from AWR report."""
    records = []
    ...
    section = soup.find("h3", string="Segments by Physical Writes")
    if not section:
        logger.warning("⚠️ Segments by Physical Writes section not found.")
        return []
    ...
    for _, row in df.iterrows():
        rec = {
            ...
            "physical_writes": clean_number(row.get("Physical Writes", None)),
            "pcttotal": clean_number(row.get("%Total", None)),
        }
        rec["row_hash"] = row_hash(rec)
        records.append(rec)

    logger.info(f"✅ Parsed {len(records)} Segments by Physical Writes records")
    return records


def insert_awr_seg_phy_writes(records):
    """Insert parsed Segments by Physical Writes into DB."""
    if not records:
        logger.warning("⚠️ No records to insert into awr_seg_phy_writes")
        return
    ...
    try:
        ...
        conn.commit()
        cur.close()
        logger.info(f"✅ Inserted {len(records)} rows into awr_seg_phy_writes")

    except psycopg2.errors.UniqueViolation:
        logger.warning("⚠️ Skipped duplicate Segments by Physical Writes record")
        if conn:
            conn.rollback()

    except Exception as e:
        logger.error(f"❌ Failed inserting Segments by Physical Writes: {e}", exc_info=True)
    finally:
        if conn:
            conn.close()
...
if __name__ == "__main__":
    ...
    if _target:
        records = parse_awr_seg_phy_writes(_target)
        insert_awr_seg_phy_writes(records)
    else:
