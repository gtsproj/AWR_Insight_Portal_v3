...
log_file = os.path.join(LOG_DIR, "sql_reads_parser.log")

logger = get_logger("sql_reads_parser")  # or dynamic module name

def parse_sql_reads(filepath):
    """Parse the SQL ordered by Reads section from AWR report."""
    records = []
    ...
    section = soup.find("h3", string="SQL ordered by Reads")
    if not section:
        logger.warning("⚠️ SQL ordered by Reads section not found.")
        return []

    table = section.find_next("table")
    if not table:
        logger.warning("⚠️ SQL ordered by Reads table not found.")
        return []

    df = pd.read_html(str(table))[0]
    logger.info(f"Found {len(df)} rows in SQL Reads section")

    if is_section_empty(df, "SQL ordered by Reads", "awr_sql_reads"):
    ...
    for _, row in df.iterrows():
        rec = {
            ...
            "physical_reads": clean_number(row.get("Physical Reads")),
            "reads_per_exec": clean_number(row.get("Reads per Exec")),
        }
        rec["row_hash"] = row_hash(rec)
        records.append(rec)

    logger.info(f"✅ Parsed {len(records)} SQL Reads records")
    return records


def insert_sql_reads(records):
    """Insert parsed SQL Reads stats into DB."""
    if not records:
        logger.warning("⚠️ No records to insert into awr_sql_reads")
        return
    ...
    try:
        ...
        insert_query = """
            ...
            VALUES (
                ...
                %(elapsed_time_s)s, 
                %(physical_reads)s, %(reads_per_exec)s,
        ...
        conn.commit()
        cur.close()
        logger.info(f"✅ Inserted {len(records)} rows into awr_sql_reads")

    except psycopg2.errors.UniqueViolation:
        logger.warning("⚠️ Skipped duplicate SQL ordered by Reads record")
        if conn:
            conn.rollback()

    except Exception as e:
        logger.error(f"❌ Failed inserting SQL ordered by Reads: {e}", exc_info=True)
    finally:
        if conn:
            conn.close()
...
if __name__ == "__main__":
    ...
    if _target:
        records = parse_sql_reads(_target)
        insert_sql_reads(records)
    else:
