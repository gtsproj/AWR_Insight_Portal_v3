#modules/sql_cpu_time_parser.py
...
log_file = os.path.join(LOG_DIR, "sql_cpu_time_parser.log")

logger = get_logger("sql_cpu_time_parser")  # or dynamic module name

def parse_sql_cpu_time(filepath):
    """Parse the SQL ordered by CPU Time section from AWR report."""
    records = []
    ...
    section = soup.find("h3", string="SQL ordered by CPU Time")
    if not section:
        logger.warning("⚠️ SQL ordered by CPU Time section not found.")
        return []

    table = section.find_next("table")
    if not table:
        logger.warning("⚠️ SQL ordered by CPU Time table not found.")
        return []

    df = pd.read_html(str(table))[0]
    logger.info(f"Found {len(df)} rows in SQL CPU Time section")

    if is_section_empty(df, "SQL ordered by CPU Time", "awr_sql_cpu_time"):
    ...
    logger.info(f"✅ Parsed {len(records)} SQL CPU Time records")
    return records


def insert_sql_cpu_time(records):
    """Insert parsed SQL CPU Time stats into DB."""
    if not records:
        logger.warning("⚠️ No records to insert into awr_sql_cpu_time")
        return
    ...
    try:
        ...
        conn.commit()
        cur.close()
        logger.info(f"✅ Inserted {len(records)} rows into awr_sql_cpu_time")

    except psycopg2.errors.UniqueViolation:
        logger.warning("⚠️ Skipped duplicate SQL ordered by CPU Time record")
        if conn:
            conn.rollback()

    except Exception as e:
        logger.error(f"❌ Failed inserting SQL ordered by CPU Time: {e}", exc_info=True)
    finally:
        if conn:
            conn.close()
...
if __name__ == "__main__":
    ...
    if _target:
        records = parse_sql_cpu_time(_target)
        insert_sql_cpu_time(records)
    else:
