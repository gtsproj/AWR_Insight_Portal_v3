...
CREATE UNIQUE INDEX IF NOT EXISTS idx_awr_sql_summary_mv_id ON awr_sql_summary_mv (mv_id) TABLESPACE awrparser_idx;

CREATE INDEX IF NOT EXISTS idx_elapsed ON awr_sql_elapsed_time (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
CREATE INDEX IF NOT EXISTS idx_cpu     ON awr_sql_cpu_time (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
CREATE INDEX IF NOT EXISTS idx_uio     ON awr_sql_user_io_time (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
CREATE INDEX IF NOT EXISTS idx_gets    ON awr_sql_gets (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
CREATE INDEX IF NOT EXISTS idx_reads   ON awr_sql_reads (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
CREATE INDEX IF NOT EXISTS idx_phy     ON awr_sql_phy_reads_unopt (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
CREATE INDEX IF NOT EXISTS idx_parse   ON awr_sql_parsed_calls (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
CREATE INDEX IF NOT EXISTS idx_cluster ON awr_sql_cluster_wait_time (dbname, instance, sql_id, begin_snap) tablespace awrparser_idx;
...
CREATE INDEX IF NOT EXISTS idx_fg_wait_summary_dbinstsnap
...
CREATE INDEX IF NOT EXISTS idx_bg_wait_summary_dbinstsnap
...
CREATE INDEX IF NOT EXISTS idx_wait_summary_dbinstsnap
...
CREATE INDEX IF NOT EXISTS idx_sar_cpu_host_time ON sar_cpu_stats (hostname, snap_time);
CREATE INDEX IF NOT EXISTS idx_sar_cpu_iowait    ON sar_cpu_stats (hostname, iowait_pct DESC);
...
CREATE INDEX IF NOT EXISTS idx_sar_mem_host_time ON sar_memory_stats (hostname, snap_time);
...
CREATE INDEX IF NOT EXISTS idx_sar_swap_host_time ON sar_swap_stats (hostname, snap_time);
...
CREATE INDEX IF NOT EXISTS idx_sar_disk_host_time ON sar_disk_stats (hostname, snap_time);
CREATE INDEX IF NOT EXISTS idx_sar_disk_util      ON sar_disk_stats (hostname, util_pct DESC);
...
ALTER TABLE awr_sql_executions         ADD COLUMN IF NOT EXISTS pdb_name TEXT;
 ALTER TABLE awr_sql_parsed_calls  ADD COLUMN IF NOT EXISTS pdb_name TEXT;  
 -- ALTER TABLE awr_sql_sharable_mem       ADD COLUMN IF NOT EXISTS pdb_name TEXT;  -- table not in schema  -- table does not exist in this schema, skipped
 -- ALTER TABLE awr_sql_version_count      ADD COLUMN IF NOT EXISTS pdb_name TEXT;  -- table not in schema  -- table does not exist in this schema, skipped
...
--ALTER TABLE awr_seg_table_scans        ADD COLUMN IF NOT EXISTS pdb_name TEXT;  -- table not in schema
...
-- AWR anomalies — Oracle DB metrics
...
awr_anomalies (
    ...
    begin_snap      INT         NOT NULL,
    snap_time       TIMESTAMP,
    metric_source   TEXT        NOT NULL,  -- wait_event|wait_class|sql_elapsed|sql_cpu|segment_lr|segment_pr
...
CREATE INDEX IF NOT EXISTS idx_awr_anomaly_db_snap ON awr_anomalies (dbname, instance, begin_snap, severity);
CREATE INDEX IF NOT EXISTS idx_awr_anomaly_time    ON awr_anomalies (snap_time DESC);

-- SAR anomalies — Linux OS metrics
CREATE TABLE IF NOT EXISTS sar_anomalies (
    id              SERIAL PRIMARY KEY,
    hostname        TEXT        NOT NULL,
    snap_time       TIMESTAMP   NOT NULL,
    metric_source   TEXT        NOT NULL,  -- sar_cpu|sar_disk|sar_mem|sar_paging|sar_load|sar_net
    metric_name     TEXT        NOT NULL,
    object_name     TEXT,                  -- cpu id, device name, iface name etc.
    metric_value    NUMERIC,
    baseline_mean   NUMERIC,
    baseline_stddev NUMERIC,
    z_score         NUMERIC,
...
CREATE INDEX IF NOT EXISTS idx_sar_anomaly_host_time ON sar_anomalies (hostname, snap_time DESC, severity);
CREATE INDEX IF NOT EXISTS idx_sar_anomaly_severity  ON sar_anomalies (severity, snap_time DESC);
...
-- =============================================================================
-- SECTION 5: CDB/PDB SUPPORT — pdb_name column on all AWR metric tables
-- (NULL for non-CDB databases; populated when parsing CDB AWR reports)
-- =============================================================================
...
--ALTER TABLE awr_os_statistics          ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_foreground_wait_class  ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_sql_elapsed_time  ts     ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_sql_cpu_time           ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_sql_user_io_time       ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_sql_gets               ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_sql_reads              ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_sql_executions         ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_sql_parsed_calls       ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_sql_cluster_wait_time  ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_sql_text               ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_instance_activity_stats ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_iostat_function        ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_iostat_filetype        ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_iostat_func_filetype   ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_tablespace_io_stats    ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_file_io_stats          ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_advisory_sga           ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_advisory_pga           ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_buffer_wait_statistics ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_enqueue_statistics     ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_undo_statistics        ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_seg_logical_reads      ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_seg_phy_reads          ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_seg_phy_read_req       ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_seg_phy_writes         ADD COLUMN IF NOT EXISTS pdb_name TEXT;
...
--ALTER TABLE awr_seg_direct_phy_reads   ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_seg_direct_phy_writes  ADD COLUMN IF NOT EXISTS pdb_name TEXT;
ALTER TABLE awr_seg_unopt_reads        ADD COLUMN IF NOT EXISTS pdb_name TEXT;
--ALTER TABLE awr_seg_opt_reads          ADD COLUMN IF NOT EXISTS pdb_name TEXT;
...
ALTER TABLE awr_sql_phy_reads_unopt    ADD COLUMN IF NOT EXISTS pdb_name TEXT;
...
CREATE TABLE IF NOT EXISTS exec_plan_headers (
    ...
    plan_type       VARCHAR(20) DEFAULT 'baseline',
    plan_text_raw   TEXT,
    sql_text        TEXT,
    total_cost      NUMERIC,
    step_count      INTEGER,
...
CREATE INDEX IF NOT EXISTS idx_plan_hdr_sql    ON exec_plan_headers(sql_id);
CREATE INDEX IF NOT EXISTS idx_plan_hdr_dbname ON exec_plan_headers(dbname);
CREATE INDEX IF NOT EXISTS idx_plan_hdr_created ON exec_plan_headers(created_at DESC);

=====================================================================================

-- ============================================================
-- AWR Insight Portal — Settings & Auth Schema
-- Run: psql -U postgres -d postgres -f portal_settings_schema.sql
-- ============================================================

-- Portal users (admin + viewer roles)
...
-- Portal configuration (key-value store)
CREATE TABLE IF NOT EXISTS portal_config (
    key          VARCHAR(100) PRIMARY KEY,
    value        TEXT,
    description  VARCHAR(300),
    section      VARCHAR(50),   -- ai | anomaly | awr_source | sar_source | license | access
...
('ai_local_model',     'llama3.1', 'Ollama model name', 'ai'),
('ai_cloud_provider',  'claude',   'Cloud AI: claude | openai | gemini | azure_openai', 'ai'),
('ai_cloud_api_key',   '',         'Cloud AI API key (stored encrypted)', 'ai'),
('ai_cloud_model',     '',         'Cloud AI model name (e.g. claude-sonnet-4-6)', 'ai'),
('ai_bfsi_warning',    'true',     'Show BFSI data residency warning for cloud AI', 'ai'),

-- Anomaly Detection
('anomaly_z_threshold',  '1.5',  'Z-score threshold for anomaly detection', 'anomaly'),
('anomaly_baseline_days','90',   'Baseline window in days', 'anomaly'),
('anomaly_min_samples',  '3',    'Minimum samples required for detection', 'anomaly'),
...
('awr_network_path',   '',       'Network/UNC path to AWR reports', 'awr_source'),
('awr_db_host',        '',       'Oracle DB host for direct connection', 'awr_source'),
('awr_db_port',        '1521',   'Oracle DB port', 'awr_source'),
('awr_db_service',     '',       'Oracle DB service name', 'awr_source'),
('awr_db_user',        '',       'Oracle DB username', 'awr_source'),
('awr_db_password',    '',       'Oracle DB password (stored encrypted)', 'awr_source'),

-- SAR Source
('sar_source_type',    'local',  'SAR source: local | ssh', 'sar_source'),
('sar_local_path',     'sar_drop','Local SAR drop folder path', 'sar_source'),
('sar_ssh_host',       '',       'SSH server hostname/IP', 'sar_source'),
('sar_ssh_port',       '22',     'SSH port', 'sar_source'),
('sar_ssh_user',       '',       'SSH username', 'sar_source'),
('sar_ssh_key_path',   '',       'Path to SSH private key', 'sar_source'),
('sar_ssh_remote_path','',       'Remote path to SAR files on server', 'sar_source'),

-- License
('license_key',        '',       'Portal license key', 'license'),
('license_db_count',   '5',      'Number of licensed Oracle databases', 'license'),
('license_sar_count',  '5',      'Number of licensed SAR servers', 'license'),
('license_expiry',     '',       'License expiry date (YYYY-MM-DD)', 'license'),
('license_customer',   '',       'Licensed customer name', 'license'),

-- Access Control
('portal_login_required', 'false', 'Require login to access portal', 'access'),
('session_timeout_mins',  '480',   'Session timeout in minutes (0=never)', 'access')
...
VALUES ('admin', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMeSSSmKPh0Z6M6GVb6LQDanPO', 'admin', 'Portal Administrator')
ON CONFLICT (username) DO NOTHING;

INSERT INTO portal_config (key, value, description, section)
VALUES ('metadata_refresh_days', '14', 'Metadata refresh frequency in days', 'access')
ON CONFLICT (key) DO NOTHING;